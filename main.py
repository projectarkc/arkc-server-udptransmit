...
def process_msg(*msg):
    ...
    salt = os.urandom(16)
    main_pw_hex="%X" % main_pw
    required_hex = "%X" % min((number), 255)
    sign_hex = '%X' % localpri.sign(salt, None)[0]
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        if msg:
            logging.info("Received request from"+addr)
        try:
        ...
        processed_msg=process_msg(decrypted_msg)
        s.sendto(processed_msg, addr)
